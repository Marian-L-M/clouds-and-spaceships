...
/** A normalised gallery image stored in block attributes. */
...
export type MultiImageAttributes = {
  images: GalleryImage[];
  maxWidth: number;
  thumbsPerRow: number;
};

function normaliseMedia(media: WPMedia): GalleryImage {
  const s = media.sizes ?? {};
...
export default function Edit({
  attributes,
  setAttributes,
}: BlockEditProps<MultiImageAttributes>) {
  ...
  const blockStyle: CSSProperties = maxWidth
    ? { maxWidth: `${maxWidth}px`, margin: "0 auto" }
    : {};
        ...
        <InspectorControls>
          <PanelBody title={__("Display", "cns-theme")} initialOpen={true}>
            ...
            <RangeControl
              ...
              value={thumbsPerRow}
              onChange={(v) => setAttributes({ thumbsPerRow: v ?? 1 })}
              min={2}
              max={10}
              __nextHasNoMarginBottom
              __next40pxDefaultSize
  ...
  return (
    <div {...blockProps}>
      ...
      <div
        ...
        style={{ "--thumbs-per-row": thumbsPerRow } as CSSProperties}
      >
        {images.map((img, i) => (
          <div
            key={img.id ?? i}
            className={`multi-image__thumb-wrap${
          ...
          >
            ...
            <div className="multi-image__thumb-actions">
              ...
              <MediaUploadCheck>
                <MediaUpload
                  onSelect={(media: WPMedia) => replaceImage(i, media)}
                  allowedTypes={["image"]}
                  value={img.id}
                  render={({ open }) => (
                    <button
