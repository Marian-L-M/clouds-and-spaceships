...
// To do fix:
// Structure and liguc is a messe
// Clean up react buttons and html buttons mixed
// Clean up inline style madness
...
export default function Edit({ attributes, setAttributes }) {
  ...
  function renderNavItem(item) {
    ...
    return (
      <li
        key={item.id}
        className={`cns-sidebar__item${isGroup ? " cns-sidebar__item--group" : ""}`}
      >
  ...
  return (
    ...
    >
      ...
      {mode === "toggle" && (
        <div
          className="cns-sidebar__toggle-btn"
          role="button"
          ...
          <ul className="cns-sidebar__links">
            {topLevelItems.length === 0 && (
              <li className="cns-sidebar__empty">
                {__(
                  'No links yet — use "Add link" in the toolbar.',
                  "cns-theme",
                )}
              </li>
            )}
